...
**Your Tobii Eye Tracker 5, working on Linux** — a native runtime and GTK4 app
in Rust, with no Tobii software installed.

[![CI](https://github.com/Tropaion/Tobii_Linux/actions/workflows/ci.yml/badge.svg)](https://github.com/Tropaion/Tobii_Linux/actions/workflows/ci.yml)
[![Latest release](https://img.shields.io/github/v/release/Tropaion/Tobii_Linux)](https://github.com/Tropaion/Tobii_Linux/releases)
...
half — the USB protocol, reimplemented from scratch, plus the app you set the
tracker up in.

It is for anyone who owns an ET5 and runs Linux: to track gaze, to calibrate the
device properly, or to put head tracking into a game.

> **Unofficial.** Not affiliated with, endorsed by, or supported by Tobii. No
> Tobii code was copied: the protocol was mapped from this project's own USB
> captures, cross-checked against the third-party
> [`tobiifree`](https://github.com/Aetherall/tobiifree) project, with op names
> and enum orderings read from a decompile of Tobii's software. Use at your own
> risk.

## What you get

- **Gaze streaming** over USB — 2D gaze point, per-eye validity, eye position in
  the trackbox, eye origins in millimetres, pupil size.
- **Follow-the-dot calibration** — 7 points in three groups, applied on the
  device and re-applied on every connect. Measured on a 49" 32:9 panel:
  **12.1 mm** mean error inside the device's usable ±28°, about 0.92° at 750 mm.
- **Guided display setup** — drag two lines onto the marks at the ends of the
  tracker, seeded from your monitor's EDID, and the screen geometry is derived.
- **Head tracking for games** — 5 DOF straight away; the 6th (pitch) needs an
  optional model download whose weights are non-commercial-only, so it is not
  shipped and is fetched only after you have seen the terms and agreed.
- **Output games can actually read** — a virtual joystick with nothing else
  installed, opentrack's UDP protocol, or TrackIR/FreeTrack through a Wine
  bridge (64-bit games only).
...
<!-- Screenshots still wanted, each needing someone at the tracker:
     docs/images/calibration.png — the follow-the-dot flow mid-run, one group of
       stimulus dots on the dark fullscreen, progress counter visible
     docs/images/display-setup.png — the guided setup, the two draggable lines on
       the marks at the ends of the tracker
     docs/images/overlay-and-tray.png — the gaze overlay dot over a desktop, with
       the status-area icon in the panel -->
...
[Releases](https://github.com/Tropaion/Tobii_Linux/releases), then install it,
re-plug the tracker, and run it.

**Debian, Ubuntu, Mint, Pop!\_OS**

```sh
sudo apt install ./tobii-linux_*.deb
# unplug and re-plug the Eye Tracker 5 — that is what activates the udev rule
tobii-gtk
```

**Fedora, RHEL** (`gtk4-layer-shell` is in EPEL) **· openSUSE** (`sudo zypper install`)

```sh
sudo dnf install ./tobii-linux-*.rpm
# re-plug the Eye Tracker 5
tobii-gtk
```

**Arch, CachyOS, Manjaro, EndeavourOS** — a prebuilt package, nothing to
compile. Every release from v0.3.1 on has one; v0.3.0 has only the `PKGBUILD`.
Download it rather than passing the URL to `pacman`: `pacman -U` checks a remote
file at `RemoteFileSigLevel`, which Arch's stock config sets to `Required`, and
this package is not signed.

```sh
# replace vX.Y.Z / X.Y.Z with the newest tag on the releases page
...
sha256sum -c --ignore-missing SHA256SUMS   # proves the file arrived intact; not who built it
sudo pacman -U ./tobii-linux-bin-X.Y.Z-1-x86_64.pkg.tar.zst
```

That checksum says the download arrived intact and nothing about who produced
it; [What installing an update trusts](#what-installing-an-update-trusts) is the
whole story.

**Any distribution, from source** — see [Building from
source](#building-from-source). On Arch that is four commands.

### What to expect the first time

- **Re-plug the tracker after installing.** The udev rule only takes effect on
  the next plug, and without it the device is reachable as root only — the
  symptom is a tracker that reports no eyes at all. See [Device
  access](#device-access) if that happens.
- **The hub opens** on `tobii-gtk`, or from *Tobii Eye Tracker* in your
  application menu.
- **The tracker stays dark** until something asks for data, and the hub says
  **Tracker off** while nothing does. That is the resting state, not a fault.
- If the tracker has never been set up on this machine, run **Change screen**
  (the *Set up display* button) and then **Improve my calibration**, in that
  order.

<details>
<summary><b>The other ways to install a release, and what each one costs</b></summary>

The choice is really one question: **do you want the built-in updater, or do you
want your system to own this?**
...
| **From source** | [the steps below](#building-from-source) | `git pull` and rebuild |
...
**On Arch**, for a pre-release, drop the dash from the version in the package's
name only: `v1.0.0-rc1` ships `tobii-linux-bin-1.0.0rc1-1-x86_64.pkg.tar.zst`,
because a pacman version cannot contain one. The tag in the URL keeps it. Once
`tobii-linux-bin` is on the AUR, `paru -S tobii-linux-bin` (or `yay -S`) does the
same and updates it along with the rest of your system. **To build it yourself**
instead, download `PKGBUILD` *and* `tobii-linux.install` from the release into
one directory and run `makepkg -si`. That compiles the program against your own
GTK (it needs `rust`) and installs a package called `tobii-linux`. It and
`tobii-linux-bin` conflict, so installing either one offers to remove the other.

**Why a prebuilt binary might refuse to install.** They are built in a Debian 13
container, which caps the glibc they can need at **2.41**. What they actually
need is measured from the binaries when they are built — **2.39** as of v0.3.0 —
and the `.deb`, the `.rpm` and the Arch package declare that measured floor, so
your package manager refuses rather than installing something that cannot start.
...
```sh
tobii-gtk                # or: Tobii Eye Tracker, in your application menu
tobii-gtk --background   # resident, no window, tracker off — with a status-area
                         # icon where the desktop has one
tobii-gtk --version
```
...
band and per eye, with a diagnosis.
...
<details>
<summary><b>Start at login</b></summary>

*Start when I log in* — behind the cogwheel, with the other settings that are
about the program rather than the tracker — writes an XDG autostart entry that
runs `tobii-gtk --background`: no window, and — because nothing is asking for
data — no tracker either. Where your desktop has a status area there is an icon
in it, which is the way to open the hub; otherwise the application menu is. It
exists so the hub is already running when you want it, and so that launching the
app hands off to the one process that can claim the tracker rather than starting
a second.
...
```sh
tobii stream [--json] [--eyes]   # decoded gaze samples
tobii setup                      # interactive display setup
tobii display get|set            # read / re-apply display area
tobii enabled-eye [both|left|right]
tobii update [--install]         # check for a new release
tobii uninstall [--dry-run]      # remove an install made by install.sh
tobii debug                      # the report an issue asks for
tobii record [--calibration]     # re-record a replay fixture
tobii --version                  # what this build reports
```

These assume `tobii` is installed and on your `PATH`; straight out of a build
tree it is `./target/release/tobii`.

Run `tobii` with no arguments for the full list, including the protocol
diagnostics (`streams`, `log`, `dump-stream`, `camera`, `cal-blob`,
`cal-points`, `probe-stream`, `columns`).
</details>
...
<details>
<summary><b>Games that speak TrackIR or FreeTrack (Wine)</b></summary>
...
**Curved panels:** the device is told the EDID **arc** width — it only accepts a
flat plane, and sending the chord makes that plane too narrow and compresses
gaze toward the edges — and the per-user calibration absorbs the curve. No
runtime gaze correction is applied; one was written and then disproved on
hardware.

## Device access

Already done if you installed a package or ran `scripts/build.sh --udev`. By
hand:
...
<details>
<summary><b>If the tracker still needs root (a system without logind)</b></summary>
...
`sudo tobii uninstall --system` a `--system` install. **A copy your package
manager owns is not touched** — it prints the manager's own command instead:
...
<details>
<summary><b>Exactly where it looks, and what it refuses to touch</b></summary>
...
The hub asks GitHub for the latest release when its window opens — the one
...
line. **Nothing is downloaded until you choose to update**, and the check can be
switched off: the switch under *Check for updates*, or `TOBII_NO_UPDATE_CHECK=1`
in the environment, which also wins over the switch. An explicit `tobii update`
still works; only the automatic check is affected.
...
It fetches the artifact that matches your manager — the `.deb`, the `.rpm`, the
prebuilt Arch package, or for a release without one the `PKGBUILD` and its
install hook together — into a version-named folder inside one you choose, and
prints the one command that installs it. It never installs anything itself. This
is the hub only: `tobii update` on the command line still refuses, and tells you
why.
...
<details>
<summary><b>What it does protect against, and the five copies that get a different button</b></summary>
...
Works on any distribution and builds against your own GTK. `scripts/build.sh`
checks the dependencies before it starts, so a missing package is named in the
first second rather than as a wall of linker errors five minutes in.
...
<details>
<summary><b>Any other distribution, and what the build flags do</b></summary>
...
`--system` for `/usr/local/bin` and every user's menu. Every user's menu entry
runs what is in that directory, so `--system` refuses one that another account
could change, and names the folder that stops it;
[Development](docs/wiki/Development.md) has the rule. Run it as yourself, not
with `sudo` — it refuses, and asks for `sudo` itself for the steps that need it.
Leave both off to just build into `target/release/`. `--install` also adds
**Tobii Eye Tracker** to your application menu.
...
- **Rust** (stable, edition 2021) — e.g. via [rustup](https://rustup.rs) — to
  build from source.
...
- A **Wayland** session is recommended: the gaze overlay is a `layer-shell`
  surface, and layer-shell has no X11 equivalent.

## Architecture and protocol documentation

[`docs/wiki/`](docs/wiki/) (mirrored to the GitHub project wiki) holds both.
...
<details>
<summary><b>The crates, in one table</b></summary>

A Cargo workspace of focused crates, one acyclic dependency graph:
...
from real executables (`crates/tobii-update/tests/install_end_to_end.rs`), and
the listing and asset-selection halves have run against two real published
...
<details>
<summary><b>Publishing a release (maintainer)</b></summary>
...
non-commercial-only). The FreeTrack and TrackIR interfaces are public ABIs;
opentrack was read for interface facts, and the files that use them say so and
say which fact. No opentrack code was copied. The StatusNotifierItem interface is
KDE's published D-Bus specification. No Tobii code was copied: the ET5's USB
protocol was mapped from this project's own USB captures, cross-checked against
tobiifree, with op names and enum orderings read from a decompile of Tobii's
software.
