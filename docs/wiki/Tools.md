...
| `tobii game -- <command>` | Run a command with the hub's tracker held on for its lifetime. It carries **no tracking data**: it connects to the hub's socket, subscribes to pose, and drops the subscription when the child exits, which is what makes the hub light the tracker while a game runs (`outputs::Holds::hello`). Needed only for the hub's route — `tobii headpose` opens the device itself and needs no wrapper. Transparent to the launcher: the child's exit code, 128 + signal for a kill, and never a refusal to start. See [[Game-Output]]. |
| `tobii games` | Print the game-output settings and the path of the file they live in. |
| `tobii games set KEY VALUE` | Change one setting. Writes through the same `apply_key` the file parser uses, so the CLI cannot accept a spelling the file would reject. `joystick`, `opentrack`, `bridge_port`, `enabled`, `rate_hz`, Extended View tuning — a rejected key prints the full list. See [[Game-Output]]. |
