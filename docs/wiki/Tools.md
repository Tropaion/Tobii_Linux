...
| `tobii bridge games` | List Steam libraries and installed games, marking which have a Proton prefix. |
| `tobii bridge install --steam <app id or name>` | Install the Wine-side DLLs into a Steam game's Proton prefix, resolved from Steam's own library files, and write the registry with **the Proton build that prefix records** (a foreign wine would upgrade the prefix). `--prefix PATH` for anything not Steam. Nothing has to be left running afterwards — the DLL receives tracking itself. See [[Game-Output]]. |
| `tobii bridge run --prefix PATH` | Optional. A console showing what arrives and what is rejected — and the one thing still *required* when TrackIR is pointed at a third-party client DLL, which is a pure consumer of the shared memory our own DLLs create. |
| `tobii game -- <command>` | Run a command with the hub's tracker held on for its lifetime. It carries **no tracking data**: it connects to the hub's socket, subscribes to pose, and drops the subscription when the child exits, which is what makes the hub light the tracker while a game runs (`outputs::Holds::hello`). Needed for the hub's route to the virtual joystick and the Wine bridge; for opentrack the hub sees the receiver's own socket instead (`wake_for_opentrack`), and `tobii headpose` opens the device itself. Transparent to the launcher: the child's exit code, 128 + signal for a kill, and never a refusal to start. See [[Game-Output]]. |
| `tobii games` | Print the game-output settings and the path of the file they live in. |
| `tobii games set KEY VALUE` | Change one setting. Writes through the same `apply_key` the file parser uses, so the CLI cannot accept a spelling the file would reject. `enabled`, `rate_hz`, `filter_alpha` and `filter_max_step_mm` (the pose filter's smoothing and its jump limit), `opentrack`, `wake_for_opentrack` (the opentrack-port watch), `bridge_port`, `joystick` and its full-swing angles, plus the Extended View tuning — `tobii games` prints the file with every key in it, and a rejected key prints the full list. See [[Game-Output]]. |
| `tobii headpose [--udp ADDR] [--rate HZ] [--model auto\|off\|FILE] [--check] [--recenter]` | Stream head pose to opentrack over UDP (default `127.0.0.1:4242`, 60 Hz). A **complete route on its own**: it opens the tracker, and anything that binds that port — opentrack's UDP input, X-Plane's `headtrack` plugin — receives it with no hub and no `tobii game`. With a model installed this is **6 DOF** — position from the eye origins, rotation from the neural model; without one it is the 5-DOF geometry and **pitch is 0**. Sinks beyond opentrack — the Wine bridge port, the virtual joystick, Extended View — are added only when `games.toml` has `enabled = true` (or `--extended-view` for the one run), so a bare invocation is what it always was: head pose to opentrack. `--check` prints the model's yaw/roll beside the geometry's, which is the experiment that settles the sign conventions. `--recenter` (`--recentre` too) takes a rotation reference at the start of the run. See [[Head-Pose]]. |
